...
void __attribute__((constructor)) setup(){
  ...
  //there is a difference between the size of the meta data and the size of the data
  ...
  //the info is also allocated to the heap
...
//offsets are calculated by calculating the offset to the metadata_heap
...
char* unlink_(chunk_metadata* curr, chunk_metadata* prev, size_t size, bin* free_list) {
  ...
  //chunks are considered in sizes of 0x20
...
char* not_malloc(size_t size) {
  ...
  // first check in the quick bins, then check in the non free bins
  char* chunk = NULL;
...
void not_free(char* chunk){
  ...
  //if I overwrite the size must be a multiple of 0x20
