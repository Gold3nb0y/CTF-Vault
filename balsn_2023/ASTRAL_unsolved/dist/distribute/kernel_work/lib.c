...
void mset(uint8_t* dst, uint8_t val, uint64_t size){
...
uint64_t strlen_local(uint8_t *msg) {
  uint8_t *c = msg;
  uint64_t l = 0; 
  for (c = msg; *c != '\0'; c++, l++);
  return l;
...
void *memcpy_local(uint8_t *dst, uint8_t *src, uint64_t size) {
  uint8_t *origBuf = dst;
...
void printStr(uint8_t *buf) {
  write(STDOUT_FILENO, buf, strlen_local(buf));
  return;
}
