...
typedef struct __attribute__((__packed__)) pkt_inst_add_t {
  uint8_t identifier;
  long index; //negative values allowed
  uint8_t has_body;
  uint16_t len;
} pkt_inst_add_t;
...
//packets are sent by first writing the header, followed by writing the body. There is no trailer
void cl_send_pkt(int sockfd, uint8_t* packet, char *body, uint16_t body_len){
    //deterined based on the type of the packet
    ...
    if (has_body){
        ...
        //set the body length and then write the body
...
//handle login, takes fd to 'server'
void hello(int fd){
    ...
    body_buf[strcspn(body_buf, "\n")] = 0; //replace the new line, can set 2 or more newlines? probably not
...
//looks very similar to login, but handles signup instead
//response freeing is handled in the recieving code
void signup(int fd){
  pkt_inst_helo_t header = {
    PKT_SIGNUP,
    1
...
void add_pw(int fd){
  ...
  //password can be a max size of 0x100
...
void report(int fd){
  ...
  header.len = body_len;//set to the bytes read
  puts("Pick a title: ");
...
//data and *packet are the same pointers
...
//write the status, the length, and the body
...
void srv_handle_helo(int fd, uint8_t* packet, uint16_t packet_len){
  ...
  for(int i = 0; i < MAX_USERS; i++){
    ...
    // hex_dump(10, packet);
    // if the size and the username does not match continue
    if( packet_len != srv_users[i]->username_len || strncmp(srv_users[i]->username, (char *) packet, packet_len))
        continue;
    //match
    exists = i;
    break;
...
void srv_handle_signup(int fd, uint8_t* packet, uint16_t packet_len){
    ...
    for(int i = 0; i < MAX_USERS; i++){
        if(!srv_users[i]) {
            //if a space has already been selected don't reselect it
            if(free_space == -1){
                free_space = i;
            }
            continue;
        }
        //notably, if a free space is found before the matching username, this check will be skipped
        //this means it may be posible to 
        //things are never removed from this list so, it is unlikely this is possible
        if(packet_len == srv_users[i]->username_len &&
                !strncmp(srv_users[i]->username, (char *) packet, packet_len)) {
            srv_err(fd, "Username is taken.");
            return;
        };
    }
    //the case that there is no free space
    if(free_space == -1){
...
void srv_handle_logout(int fd, uint8_t* packet, uint16_t packet_len){
  ...
  //nothing response. the current user thing doesn't change as well
  srv_res(fd,0,0,"");
}
...
void srv_handle_get(int fd, pkt_inst_get_t* header){
  ...
  //lookup done by passwords so an index move more than 1
...
void srv_handle_del(int fd, pkt_inst_del_t* header){
    ...
    //free it, also clear the pointer
    free(pw->password);
    pw->password = 0;
    pw->len = 0;
    //return succes
...
//the bug packet doesn't do anything, or does it.....
void srv_handle_bug(int fd, pkt_inst_del_t* header, uint8_t* packet, uint16_t packet_len){
  srv_res(fd,0,0,0); //todo
}

//based on the type of the packet switch the way that it is handled
  ...
  while(len){
    ...
    switch(state->state){
      ...
      case SRV_PARSE_HDR:
        ...
        //expected length must match header length
        if(state->i == state->header_length){
          ...
          //data length is sent frim within the header, notably during then end off it
          ...
          //if there is a body and data length is properly read then allocate memory accorind to the size given
          //I have a strong feeling that the bug will be heap overflow
          else {
    ...
    //once setup operations have been performed, the packet is handled based on it's operations
    if(state->state == SRV_PARSE_DISPATCH){
      // printf("srv handling\n");
      srv_handle_pkt(fd, state->header, state->packet, state->i);
      //if a packet was malloced free it. (refrence still remains)
      //this would require the data_length and length to be different values
      //this could give a double free primative
      if(state->packet){
        free(state->packet);
      }
      //would need to find a way to overwrite the state afterwards
      state->state = SRV_PARSE_INIT;
    }
...
void srv_loop(int fd){
  ...
  while(1){
    num_rcv = 0;
    //packets are parsed 0x40 bytes at a time
