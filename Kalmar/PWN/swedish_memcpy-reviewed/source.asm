...
dq kernel_virt_end - kernel_virt_base - 8
    ...
    or byte [0x1000], 4
    or byte [0x2000], 4
    mov rdi, 0x5000 ;address to store at
    mov rax, 0x8007 ;executable space
    stosq
    mov rax, 0x9007
    stosq
    mov rcx, 0x200 - 2
    mov rax, 0x0003
.moar: ;mark inaccessible userspace
    stosq
    add rax, 0x1000
    loop .moar ;loops 0x1FE times via rcx, storing 0x3 to 0x1FE003
...
read_userspace_code:
    xor rdi, rdi ;read to 0x0
    mov rcx, 0x1000 ;max size read of 0x1000 bytes, which is what the truncat is too
...
enter_userspace:
    mov ax, 0x20 | 3 ;set the flags for userspace
    mov ds, ax
    mov es, ax
    mov fs, ax
    mov gs, ax
