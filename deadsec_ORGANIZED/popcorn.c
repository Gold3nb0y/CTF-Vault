...
void print_review_rated(void* first){
    ...
    printf("\trating: %zu\n\treview: ", rev->rating);
    fwrite((void *)rev+0x20,1,rev->length,stdout);
    puts("");
}
...
void* add_review(struct Movie* selection){
    ...
    if(answer == 'y'){
        ...
        struct Rated_Review* temp = (struct Rated_Review*)malloc(sizeof(struct Rated_Review)+length);
        temp->rating = rating;
        temp->length = length;
        ...
        new_review = (struct Review*)malloc(sizeof(struct Review)+length);
...
void edit_movie(struct Movie* to_edit){
    ...
    fread(to_edit->name, 1, 0x1f, stdin);
}
